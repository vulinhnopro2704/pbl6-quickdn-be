  ...
  public void init() {
    try (InputStream serviceAccount = resolveServiceAccountStream()) {
      ...
      if (FirebaseApp.getApps().isEmpty()) {
        ...
        log.info("✅ Firebase has been initialized (order-service)");
      } else {
        log.info("Firebase already initialized (order-service), skipping re-init");
      }
    } catch (Exception e) {
      log.error("❌ Failed to initialize Firebase (order-service)", e);
    }
  }
