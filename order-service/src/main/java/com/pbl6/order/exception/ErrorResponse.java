...
public record ErrorResponse(
    ...
    @Schema(
            ...
            required = true)
        String message,
    @Schema(description = "HTTP status code", example = "400", required = true) int status,
    ...
    @Schema(description = "Request path where the error occurred", example = "/api/order/create")
        String path,
    @Schema(
            description = "Validation errors for field-specific issues",
  ...
  public ErrorResponse(
      String error, String message, int status, String path, Map<String, String> validationErrors) {
