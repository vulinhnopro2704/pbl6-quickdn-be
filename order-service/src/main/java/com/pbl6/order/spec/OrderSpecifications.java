...
public final class OrderSpecifications {
    ...
    public static Specification<OrderEntity> hasStatus(String status) {
        return (root, query, cb) -> cb.equal(root.get("status"), Enum.valueOf(
                com.pbl6.order.entity.OrderStatus.class, status));
    }
    ...
    public static Specification<OrderEntity> freeTextSearch(String q) {
        ...
        return (root, query, cb) -> {
            ...
            // 2) search trong text fields (pickupAddress.detail, package.description, dropoffAddress.detail, dropoffAddress.name, customerNote, ...)
            try {
