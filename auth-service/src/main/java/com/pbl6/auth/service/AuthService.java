...
public class AuthService {
  ...
  public AuthResponse login(String phone, String rawPassword) {
    ...
    User u =
        userRepo
            .findByPhone(phone)
            .orElseThrow(
                () -> new UnauthorizedException("Số điện thoại hoặc mật khẩu không hợp lệ"));
  ...
  public AuthResponse refresh(String refreshToken) {
    ...
    if (!u.isEnabled() || !u.isActive()) {
      ...
      throw new UnauthorizedException(
          "Tài khoản người dùng chưa được kích hoạt hoặc đã bị vô hiệu hóa");
    }
  ...
  //            return ResponseEntity.badRequest().body(Map.of("message", "Invalid or missing
  // token"));
  //        }
  //
